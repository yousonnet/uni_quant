...
async function getBlockNumberFromTimestamp(timestamp: number): Promise<number> {
  const response = await e_scan_cli.get("", {
    params: {
      module: "block",
...
async function getInternalTxByHash(hash: string) {
  const response = (
    await e_scan_cli.get("", {
      params: {
        module: "account",
...
async function getInternalTxFromBlockRange(
  ...
  for (let i = 0; i <= mod; ++i) {
    if (i !== mod) {
      const response_promise = e_scan_cli.get("", {
        params: {
          module: "account",
          action: "txlistinternal",
  ...
  for (let axios_response of total_axios_response) {
    if (axios_response.data.result.length === 0) {
      throw new Error("no internal tx in this block range,weird,must panic!!!");
